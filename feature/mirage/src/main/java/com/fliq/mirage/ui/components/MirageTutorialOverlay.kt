...
data class MirageTutorialHighlight(
    val center: Offset,
    val size: Size,
...
@Composable
fun MirageTutorialOverlay(
    step: MirageTutorialStep,
    highlight: MirageTutorialHighlight?,
    onNext: () -> Unit,
    onSkip: () -> Unit
) {
    ...
    Box(
        modifier = Modifier
            ...
            .then(
                if (step != MirageTutorialStep.TILE_INTERACT) {
                    Modifier.clickable(
...
@Composable
fun MirageTutorialTooltip(
    step: MirageTutorialStep,
    modifier: Modifier = Modifier
) {
    val title = when (step) {
        MirageTutorialStep.WELCOME -> "WELCOME TO THE MIRAGE"
        MirageTutorialStep.STATS -> "WATCH YOUR PERFORMANCE"
        MirageTutorialStep.TILE_INTRO -> "CHANGING REALITY"
        MirageTutorialStep.TILE_INTERACT -> "GIVE IT A TRY"
        MirageTutorialStep.START_GAME -> "READY TO BEGIN?"
    }

    val description = when (step) {
        MirageTutorialStep.WELCOME -> "In this mode, nothing is what it seems. Observe the tiles carefully!"
        MirageTutorialStep.STATS -> "Your score and remaining lives are tracked here."
        MirageTutorialStep.TILE_INTRO -> "Tiles start as coins but may transform into bombs. Only tap when it's safe!"
        MirageTutorialStep.TILE_INTERACT -> "Tap the highlighted tile now to experience the mirage!"
        MirageTutorialStep.START_GAME -> "Tapping PLAY begins the session. Stay sharp!"
    }
