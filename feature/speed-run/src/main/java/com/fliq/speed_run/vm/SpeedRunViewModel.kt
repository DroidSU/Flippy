...
class SpeedRunViewModel @Inject constructor(
    ...
    private val _tutorialStep = MutableStateFlow<SpeedRunTutorialStep?>(null)
    ...
    init {
        getUserData()
        if (!settingsRepository.isSpeedRunTutorialCompleted()) {
            _tutorialStep.value = SpeedRunTutorialStep.WELCOME
        } else {
    ...
    private fun checkRotationPromptVisibility() {
        if (!settingsRepository.isSpeedRunRotationHintDisabled() && !hasShownRotationPromptInSession) {
    ...
    fun nextTutorialStep() {
        ...
        when (current) {
            SpeedRunTutorialStep.TILE_INTRO -> {
        ...
        _tutorialStep.value = when (current) {
            SpeedRunTutorialStep.WELCOME -> SpeedRunTutorialStep.STATS
            SpeedRunTutorialStep.STATS -> SpeedRunTutorialStep.TILE_INTRO
            SpeedRunTutorialStep.TILE_INTRO -> SpeedRunTutorialStep.TILE_INTERACT
            SpeedRunTutorialStep.TILE_INTERACT -> SpeedRunTutorialStep.START_GAME
            SpeedRunTutorialStep.START_GAME -> {
                settingsRepository.setSpeedRunTutorialCompleted(true)
    ...
    fun onRotationPromptDismissed(dontShowAgain: Boolean) {
        ...
        if (dontShowAgain) {
            settingsRepository.setSpeedRunRotationHintDisabled(true)
        }
    }
    ...
    fun onTileTapped(tileId: Int, tapPosition: Offset? = null) {
        if (_tutorialStep.value == SpeedRunTutorialStep.TILE_INTERACT) {
        ...
        scope.launch {
            ...
            when (tile.type) {
                CardType.COIN -> {
                    ...
                    val reactionTime = (rawReactionTime - offset).coerceAtLeast(10L) // Minimum 10ms
