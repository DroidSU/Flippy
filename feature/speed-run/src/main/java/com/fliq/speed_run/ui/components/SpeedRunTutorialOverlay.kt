...
data class SpeedRunTutorialHighlight(
    val center: Offset,
    val size: Size,
...
@Composable
fun SpeedRunTutorialOverlay(
    step: SpeedRunTutorialStep,
    highlight: SpeedRunTutorialHighlight?,
    onNext: () -> Unit,
    onSkip: () -> Unit
) {
    ...
    Box(
        modifier = Modifier
            ...
            .then(
                if (step != SpeedRunTutorialStep.TILE_INTERACT) {
                    Modifier.clickable(
...
@Composable
fun SpeedRunTutorialTooltip(
    step: SpeedRunTutorialStep,
    modifier: Modifier = Modifier
) {
    val title = when (step) {
        SpeedRunTutorialStep.WELCOME -> "WELCOME TO SPEED RUN"
        SpeedRunTutorialStep.STATS -> "WATCH YOUR SCORE"
        SpeedRunTutorialStep.TILE_INTRO -> "HIDDEN TREASURES"
        SpeedRunTutorialStep.TILE_INTERACT -> "GIVE IT A TRY"
        SpeedRunTutorialStep.START_GAME -> "READY TO BEGIN?"
    }

    val description = when (step) {
        SpeedRunTutorialStep.WELCOME -> "Fast-paced match action. Test your reflexes as the speed increases over time!"
        SpeedRunTutorialStep.STATS -> "Your score and remaining lives are tracked here."
        SpeedRunTutorialStep.TILE_INTRO -> "Once a tile is revealed, tap on coins to increase your score. Avoid bombs at all costs - they deplete your lives."
        SpeedRunTutorialStep.TILE_INTERACT -> "Tap the highlighted tile now to experience your first match!"
        SpeedRunTutorialStep.START_GAME -> "Tapping PLAY begins the session. Be fast, be accurate!"
    }
