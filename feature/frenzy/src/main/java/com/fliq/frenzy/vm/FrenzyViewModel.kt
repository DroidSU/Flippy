...
@HiltViewModel
class FrenzyViewModel @Inject constructor(
    ...
    private val exceptionHandler = CoroutineExceptionHandler { _, throwable ->
        Log.e("FrenzyViewModel", "Coroutine exception: ${throwable.localizedMessage}", throwable)
    }
    ...
    fun startGame() {
        ...
        analyticsRepository.logEvent("game_started", mapOf("challenge" to "FRENZY"))
        startTimer()
    ...
    private fun revealAndHideTile(tileId: Int) {
        // Use the gameLoopJob as the parent so this animation is cancelled if the game ends/restarts
        ...
        scope.launch(parentJob) {
            ...
            val newType = CardType.COIN // Only Coins in Frenzy
            ...
            if (tile?.isRevealed == true && tile.lastRevealTime == revealTime) {
                // In Frenzy, missing one coin ends the game
                _lives.value = 0
                endGame()
    ...
    private fun handleMissedCoin() {
        // Not used in Frenzy as it has Zero Tolerance (direct endGame)
    }
